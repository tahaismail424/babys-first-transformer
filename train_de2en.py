...
if __name__ == "__main__":
    ...
    train_set = IWSTLDataset(objective="de2en", set="train")
    test_set = IWSTLDataset(objective="de2en", set="test")
    ...
    sentence_samples = [
        ...
        "Ich hoffe wirklich, dass dieses Modell einigermaßen funktioniert...",
        "Wenn nicht, werde ich wirklich traurig sein.",
    ]

    sample_predictions(model, tokenizer, sentence_samples, bos_id, pad_id, eos_id, device)

    # save model
    torch.save(model.state_dict(), f"data/weights/transformer_de2en_{datetime.now().isoformat()}.pth")
